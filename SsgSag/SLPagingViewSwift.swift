...
open class SLPagingViewSwift: UIViewController, UIScrollViewDelegate {
    ...
    public init(items: [UIView], views: [UIView], showPageControl: Bool, navBarBackground: UIColor) {
        ...
        for (i, view) in views.enumerated() {
            ...
            //self.view.bringSubviewToFront(views[i])
    ...
    public convenience init(items: [UIView], controllers: [UIViewController], showPageControl: Bool){
        self.init(items: items, controllers: controllers, showPageControl: showPageControl, navBarBackground: UIColor.lightGray)
    }
    ...
    fileprivate func addViews() {
        if self.views.count > 0 {
            ...
            var i: Int                  = 0
            while let v = views[i] {
                v.frame          = CGRect(x: self.SCREENSIZE.width * CGFloat(i), y: 0, width: self.SCREENSIZE.width, height: self.SCREENSIZE.height - 65 )
                
                self.scrollView.addSubview(v)
  //              v.frame = self.scrollView.frame
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let distance = CGFloat(120 + self.navigationSideItemsStyle.rawValue)

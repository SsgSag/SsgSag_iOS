//
//  ViewControllerIdentifier.swift
//  SsgSag
//
//  Created by admin on 20/05/2019.
//  Copyright © 2019 wndzlf. All rights reserved.
//

import Foundation

enum ViewControllerIdentifier {
    static let swipe = "Swipe"
    static let mypageViewController = "MyPageVC"
    static let userInfoViewContrller = "UserInfoVC"
    static let loginPopUpViewController = "LoginPopUp"
    static let singupFirstViewController = "SignupFirst"
    static let applySuccessViewController = "applySucess"
    static let addPassiveDataNavigationController = "AddPassiveDateNV"
    static let detailPosterViewController = "DetailPoster"
    static let calendarDetailPopUpViewController = "CalendarDetailPopUpVC"
    static let datePickerPopUpViewController = "DatePickerPoPUp"
    static let appInfoViewController = "AppInfoViewController"
    static let addActivityViewController = "AddActivityVC"
    static let addViewController = "AddVC"
    static let addCertificationViewController = "AddCertificationVC"
    static let loginViewController = "Login"
    static let schoolInfoViewController = "SchoolInfoVC"
    static let pageViewContrller = "PageViewController"
    static let zoomPosterViewController = "ZoomPosterVC"
    static let loginNavigtaionController = "LoginNavigator"
    static let termsOfServiceViewController = "TermsOfServiceViewController"
    static let privateProtectViewController = "PrivateProtectViewController"
}
